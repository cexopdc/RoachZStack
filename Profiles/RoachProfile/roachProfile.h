...
#ifndef ROACHPROFILE_H
#define ROACHPROFILE_H
...
typedef void (*roachProfileChange_t)( uint8 paramID );

typedef struct
{
  roachProfileChange_t        pfnRoachProfileChange;  // Called when characteristic value changes
} roachProfileCBs_t;
...
extern bStatus_t RoachProfile_AddService( uint32 services );
 ...
 */
extern bStatus_t RoachProfile_RegisterAppCBs( roachProfileCBs_t *appCallbacks );
 ...
 *          uint16 pointer).
 */
extern bStatus_t RoachProfile_SetParameter( uint8 param, unsigned int len, void *value );
 ...
 *          uint16 pointer).
 */
extern bStatus_t RoachProfile_GetParameter( uint8 param, void *value );
