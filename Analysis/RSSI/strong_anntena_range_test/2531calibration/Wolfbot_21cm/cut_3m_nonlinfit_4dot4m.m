...
%%% RSSI VS distance %%%
clear all;
input = load('cut_3m_dist_mean_std_diff_rssi.txt');
rssi_list = input(:,1);
...
title('cut 3m: RSSI vs Distance fitting');

set(gca, 'FontSize', 18, 'LineWidth', 2);
exportfig(gcf,'cut_3m_rssi_dis_fit_4.4m.eps'...
    ,'height',6,'Width',8,'fontmode','Scaled', 'color', 'rgb');

%store the rssi vs distance fitting
input = load('cut_3m_interp_dist_mean_std_diff_rssi.txt');
x3 = input(:,1);
y3 = 10.^(b(1)-b(2)*x3);
std3 = input(:,3);
rssi_dis_fitting = [x3 y3 std3];
dlmwrite('cut_3m_rssi_dis_fitting.txt', rssi_dis_fitting, 'delimiter','\t');
...
hold on;
errorbar(x1,distance_mean_list,distance_std_list,'bo','markersize', 5);
title('cut 3m: RSSI vs Distance weighted fitting');
set(gca, 'FontSize', 18, 'LineWidth', 2);
exportfig(gcf,'cut_3m_rssi_dis_fit_weighted_4.4m.eps'...
    ,'height',6,'Width',8,'fontmode','Scaled', 'color', 'rgb');
...
std3 = input(:,3);
rssi_dis_fitting = [x3 y3 std3];
dlmwrite('cut_3m_rssi_dis_weighted_fitting.txt', rssi_dis_fitting, 'delimiter','\t');
