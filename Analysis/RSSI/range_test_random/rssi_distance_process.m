...
%plot random move from 0.2m to 1.0m
random_move_mean_std = [];
for i=0.2:0.2:1.0
    input = load(strcat('random_move_',char(vpa(i,2)),'m.txt'));
    input = input(1:1000,:);
    
    mean_rssi = mean(input);
    std_rssi = std(input);
 
    random_move_mean_std = [random_move_mean_std;mean_rssi std_rssi];
end

rho_mean = random_move_mean_std(:,1)';
rho_std = random_move_mean_std(:,2)';
x = 0.2:0.2:1.0;
h(3) = plot(x,rho_mean,'go','markersize', 5);
h_error=errorbar(x,rho_mean,rho_std,'go','markersize', 5);
legend(h(1:3),'Each spot','Aggregate of 5 spot','Random\_move\_everywhere');
xlim([0 3.2]);
