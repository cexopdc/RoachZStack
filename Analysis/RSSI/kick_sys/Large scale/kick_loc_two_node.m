...
%  Using two_node_kalman
%  
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function node_stat = kick_loc_two_node
global Node;
global NUM_NODE;
global STAGE_NUMBER;
global BEACON_RATIO;
...
for time = 0:STAGE_NUMBER
    Node(NUM_NODE*BEACON_RATIO+1).std = sqrt(sum(sum(Node(NUM_NODE*BEACON_RATIO+1).cov))/2);
        %Node(4).std = [sqrt(Node(4).cov(1,1));sqrt(Node(4).cov(2,2))];
    node_stat  = [node_stat;[Node(NUM_NODE*BEACON_RATIO+1).est_pos;Node(NUM_NODE*BEACON_RATIO+1).std]'];
