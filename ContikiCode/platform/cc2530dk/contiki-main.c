...
static rtimer_clock_t start_count=0, prev_count=0; /*fgong TIMING VAR*/
/*---------------------------------------------------------------------------*/
#if ENERGEST_CONF_ON
...
main(void) CC_NON_BANKED
{
  ...
  //random_init(clock_time()); //fgong_DEBUG: change 0 into clock_time() to set the seed differently for different nodes. seems like not necessary since it is different from rand()/srand().
  random_init(0);
...
#ifdef fgong_DEBUG
    if (prev_count != start_count){                    /*fgong*/
    	printf("time=%ux7.8ms",clock_time()-start_count);  /*fgong*/
	prev_count = start_count;                      /*fgong*/
    }								
#endif 				       
    len = NETSTACK_RADIO.pending_packet();
    if(len) {
      start_count = clock_time(); /*fgong: current timestamp*/
      packetbuf_clear();
#ifdef fgong_DEBUG
      printf("lenB=%u,",len); /*fgong*/
#endif
