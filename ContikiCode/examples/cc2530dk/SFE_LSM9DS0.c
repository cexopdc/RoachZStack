/******************************************************************************
SFE_LSM9DS0.c  / Modified version of the Sparkfun SFE_LSM9DS0.cpp -tmagcaya
...
uint16_t begin(enum gyro_scale gScl, enum accel_scale aScl, enum mag_scale mScl, 
...
{  
  ...
  clock_delay_ms(20);                                 // Wait for change to take effect converted to contiki specific function
  gWriteByte(FIFO_CTRL_REG_G, 0x20 | 0x1F);  // Enable gyro FIFO stream mode and set watermark at 32 samples
  clock_delay_ms(1000);///  // delay 1000 milliseconds to collect FIFO samples
  ...
  clock_delay_ms(20);                                // Wait for change to take effect
...
void readGyro()
{
	...
	// printf("unsigned int gx %u\n", (unsigned int)gx);
	// printf("unsigned int gy %u\n", (unsigned int)gy);
	// printf("unsigned int gz %u\n", (unsigned int)gz);
	//printf("refbefcast int %u\n", (unsigned int)&gx);
	//printf("refaftcast int %u\n", &(unsigned int)gx);
	// printf("temp10 %u\n", temp[0]);
	// printf("temp11 %u\n", temp[1]);
	//printf("temp12 %u\n", temp[2]);
	//printf("temp13 %u\n", temp[3]);
...
{
	spi_init();
	spi_configureChannel(0);
	spi_configureChannel(1);
...
void SPIwriteByte(uint8_t csPin, uint8_t subAddress, uint8_t data)
{
	...
	write = subAddress & 0x3F;
	spi_write(csPin, &write, 1);  //send address
	spi_write(csPin, &data, 1); // Send data
							...
							uint8_t * dest, uint8_t count)
{
	...
	if (count > 1)
		write = 0xC0 | (subAddress & 0x3F);
	else
		write = 0x80 | (subAddress & 0x3F);
	spi_write(csPin, &write, 1);
	spi_read(csPin, dest, count);
...
void clock_delay_ms(uint16_t dt){
	int ii;
	for(ii = 0; ii < dt ; ii++) {          // delay ms
