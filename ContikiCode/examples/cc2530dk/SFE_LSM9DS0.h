...
// Default: enum gyro_scale gScl = G_SCALE_245DPS, 
...
uint16_t begin(enum gyro_scale gScl, 
			enum accel_scale aScl, enum mag_scale mScl,
			enum gyro_odr gODR, enum accel_odr aODR, 
			enum mag_odr mODR);
...
// default: uint8_t int1Cfg, uint16_t int1ThsX = 0,
//					  uint16_t int1ThsY = 0, uint16_t int1ThsZ = 0, 
//					  uint8_t duration = 0
void configGyroInt(uint8_t int1Cfg, uint16_t int1ThsX,
					  uint16_t int1ThsY, uint16_t int1ThsZ, 
					  uint8_t duration);
...
// a function that I had to define. 
